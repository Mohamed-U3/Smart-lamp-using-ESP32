...
#define CLIENTLOCAL_SET_SERVER(     Micro_ENDPOINT, Micro_PORT      )           clientLocal.setServer(      Micro_ENDPOINT, Micro_PORT              )
#define CLIENT_SET_CALLBACK(        Micro_messageHandler            )           client.setCallback(         Micro_messageHandler                    )
#define CLIENTLOCAL_SET_CALLBACK(   Micro_messageHandler            )           clientLocal.setCallback(    Micro_messageHandler                    )
#define CLIENT_CONNECT(             Micro_THINGNAME                 )           client.connect(             Micro_THINGNAME                         )
#define CLIENTLOCAL_CONNECT(        Micro_THINGNAME                 )           clientLocal.connect(        Micro_THINGNAME                         )
#define CLIENT_CONNECTED(                                           )           client.connected(                                                   )
#define CLIENTLOCAL_CONNECTED(                                      )           clientLocal.connected(                                              )
#define CLIENT_SUBSCRIBE(           Micro_SUBSCRIBE_TOPIC           )           client.subscribe(           Micro_SUBSCRIBE_TOPIC                   )
#define CLIENTLOCAL_SUBSCRIBE(      Micro_SUBSCRIBE_TOPIC           )           clientLocal.subscribe(           Micro_SUBSCRIBE_TOPIC              )
